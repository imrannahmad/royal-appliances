...
export default function Navbar() {
  ...
  return (
    ...
    >
        ...
        <div className="flex items-center justify-between">
          ...
          <a href="#hero" className="flex items-center gap-2.5 group focus:outline-none shrink-0">
            ...
            <div className="flex flex-col">
              <span className="font-serif-royal text-sm sm:text-base lg:text-lg font-bold tracking-wider gold-gradient-text leading-tight whitespace-nowrap">
                THE ROYAL APPLIANCES
              </span>
              <div className="flex items-center gap-1.5 text-[9px] sm:text-[10px] tracking-widest text-slate-400 uppercase whitespace-nowrap">
          ...
          <nav className="hidden lg:flex items-center gap-5 xl:gap-7">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
                className="text-xs font-semibold tracking-wider text-slate-300 hover:text-[#FFD700] uppercase transition-colors py-1 relative group whitespace-nowrap"
              >
                {link.name}
          ...
          <div className="hidden lg:flex items-center gap-3 shrink-0">
            <div className="flex items-center gap-1.5 text-[11px] text-amber-200/90 font-medium px-3 py-1 rounded-full bg-[#141520] border border-[#D4AF37]/20 whitespace-nowrap">
              <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping" />
              <span>Doorstep Response Active</span>
            </div>
            <a
              href="tel:+919661633901"
              className="gold-gradient-bg text-[#08080A] font-bold text-xs px-4 py-2 rounded-full flex items-center gap-2 hover:opacity-95 shadow-[0_0_15px_rgba(212,175,55,0.3)] hover:shadow-[0_0_25px_rgba(212,175,55,0.6)] transform hover:-translate-y-0.5 transition-all duration-300 whitespace-nowrap"
            >
      ...
      {/* Mobile Drawer Menu */}
      {isMobileMenuOpen && (
        <div className="lg:hidden bg-[#08080A]/98 border-b border-[#D4AF37]/30 px-4 pt-4 pb-6 mt-2 space-y-3 backdrop-blur-xl animate-in slide-in-from-top duration-200">
